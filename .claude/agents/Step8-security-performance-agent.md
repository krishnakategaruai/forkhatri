---
name: step8-security-performance-agent
description: >
...
# Supplementary review requests (from Implementation, Step 9)

If Implementation raises a blocker because it needs a dependency that
wasn't part of this file's original analysis, do not treat this as
reopening the whole step. Run a **scoped STRIDE pass on just that one
dependency** — same six categories, same rigor, but bounded to the new
component/data flow it introduces — and record it as a new item in this
file (e.g. `SP0x-supplement-01`) rather than editing existing items.
Clear the specific blocker in `09a-external-dependencies.md` once done so
Implementation can seal.
