...
tools: Read, Write, Grep, Glob, Bash, Task, mcp__dbhub__execute_sql, mcp__dbhub__search_objects
model: inherit
---
...
1. **Dependency identification.** Use `Grep`/`Glob`/`Read` on the actual
   ...
   For anything database-related specifically, use
   `mcp__dbhub__search_objects` to check the real Postgres schema (via
   DBHub) rather than relying on code-grep alone — an existing table,
   column, or constraint won't show up in a code search if nothing
   references it yet. List everything found as a table, not prose — an
   unlisted dependency is the one this whole step exists to catch.
