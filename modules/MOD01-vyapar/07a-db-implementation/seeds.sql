...
-- Re-runnable: init.sh runs migrations (which DROP/CREATE per-object with
-- IF NOT EXISTS) before this file; this file itself is not drop/recreate
-- safe on its own (plain INSERTs) — re-run via a fresh ./init.sh after
-- dropping the database if you need a clean reseed (see README.md).
--
-- Migrated from modules/MOD01-vyapar/db/seeds.sql (the pre-Step-7a draft,
-- left in place for history) into the new schema-per-component structure.
-- Every row from that file is preserved; the only content change is
-- promotion_history -> commercial_order_history (TR031 gap fix — see
-- 07a-er-model.md). All explicit column-list INSERTs below still name only
-- pre-existing columns; every column 07a added (setup_step, content_language,
-- anonymized, deleted_at, etc.) has a DEFAULT and is intentionally left unset
-- by seed data.
-- =============================================================================
SET search_path TO vyapar_identity, vyapar_listings, vyapar_opportunities, vyapar_enquiries, vyapar_reviews, vyapar_commercial, vyapar_payments, vyapar_privacy, vyapar_trust_safety, vyapar_analytics, vyapar_integration, vyapar_platform, public;
...
-- [TR031 gap fix] commercial_order_history replaces the draft's promotion-only history table.
INSERT INTO commercial_order_history (order_kind, order_id, from_state, to_state, reason) VALUES
('promotion','00000000-0000-4000-8000-000000000501',NULL,'awaiting_payment','created'),('promotion','00000000-0000-4000-8000-000000000501','awaiting_payment','active','payment succeeded'),
('promotion','00000000-0000-4000-8000-000000000502',NULL,'awaiting_payment','created'),('promotion','00000000-0000-4000-8000-000000000502','awaiting_payment','active','payment succeeded'),('promotion','00000000-0000-4000-8000-000000000502','active','completed','duration ended');
...
INSERT INTO commercial_order_history (order_kind, order_id, from_state, to_state, reason) VALUES
('entitlement','00000000-0000-4000-8000-000000000601',NULL,'awaiting_payment','created'),('entitlement','00000000-0000-4000-8000-000000000601','awaiting_payment','active','payment succeeded');
