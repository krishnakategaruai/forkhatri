...
| 2026-09-14 | Post-seal correction: ForKhatri platform identity. Added dated notes to §1 (Identity edge) and §2.1 (Identity Bridge) describing how the bridge accepts the ForKhatri session per `/MODULE-ARCHITECTURE-STANDARD.md` §5b. No component, diagram or decision removed. Not re-sealed; awaits the owner's review. | Product-owner instruction, 2026-09-14. See `docs/ParentApp/07-tech-reqs.md` TR11, TR14–TR16 and `/ARCHITECTURE.md` ADR-019/ADR-021. |
...
> **2026-09-14 correction:** the "Mangaly Service → Identity & Trust Service" edge below is `POST /internal/v1/sessions/resolve` (service name + key) plus `POST /internal/v1/members/lookup`, not short-lived JWTs (ADR-019). The "Web Client" is the Mangaly web zone entered from the ForKhatri entrance (ADR-020). See docs/ParentApp/07-tech-reqs.md TR14–TR16, TR22.
...
- **Identity** is kept as a genuinely *thin bridge* component — no Mangaly-owned business logic and no schema of its own; it only translates to the platform Identity & Trust Service.
  > **2026-09-14 correction:** Identity Bridge now follows `/MODULE-ARCHITECTURE-STANDARD.md` §5b: read `fk_session`; resolve through the internal API with an in-process cache of at most 30 s; fail closed with `503`; ensure the `mangaly_identity.account` member-link row (canonical `member_id`); bind `SET LOCAL` RLS context. It exposes no sign-in endpoints; Mangaly's interim credential routes and `mangaly_session` cookie are disabled by default. Its interim `mangaly_identity` schema (07a Assumptions #1) remains as the member-link table. See docs/ParentApp/07-tech-reqs.md TR11, TR15.
