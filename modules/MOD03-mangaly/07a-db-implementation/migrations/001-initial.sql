-- =============================================================================
-- Migration 001 — Initial schema (all 14 Mangaly schemas, first production
-- migration). No prior migration to depend on — this is migration zero.
-- Idempotent: every CREATE SCHEMA/TABLE uses IF NOT EXISTS, every CREATE TYPE
-- is wrapped in a DO block that swallows duplicate_object, so this file can
-- be re-run against a database that already has it applied with no error.
-- This file is authoritative; ../schema.sql is kept as an identical snapshot
-- for fast local dev setup — see ../README.md "Migrations vs. schema.sql".
-- =============================================================================
...
COMMENT ON TABLE mangaly_platform.idempotency_key IS
  '[TR102/MODULE-ARCHITECTURE-STANDARD §4b] One shared implementation every client-queueable mutation endpoint honors — never re-derived per component.';
...
-- No RLS on mangaly_platform: rows are keyed by opaque (key, endpoint,
-- window) tuples the middleware itself scopes to the calling account/key
-- server-side before ever issuing a query — there is no "another actor's
-- row" concept to leak here the way there is for business data.
