...
| 2026-09-14 | **Profile hub v3, round 2: photo grid, prompts placed throughout, palette v3.** Three follow-ups from direct product feedback. (1) *"Where can photos be added?"* — nowhere, after signup: only the single creation-time photo existed. `profile_media` already supported many rows per profile, so added self-only `GET/POST /profile/photos`, `DELETE /profile/photos/{id}` and `POST /profile/photos/{id}/primary` (`profile.list_own_photos/add_photo/delete_photo/set_primary_photo`): a six-photo cap (Hinge's and Bumble's number), the first photo becomes main, deleting the main photo promotes the oldest remaining one, and the last photo cannot be deleted because DEC-V1-001's existence tier requires one. The hub opens on a six-slot grid (main photo 2×2) with a sheet to make a photo main or remove it; photos stay behind the connection boundary exactly as before — other viewers still only ever reach the primary photo, and only post-connection. Local-disk stand-in note: removed files are not deleted from disk; stored-byte lifecycle belongs to TR006's real Object Storage. Found live: `GET /profile/photos` initially returned `{}` because the running server had not reloaded and the request fell through to `GET /{category}`; restarted and confirmed via `/openapi.json` that `/profile/photos` is registered ahead of `/profile/{category}`. (2) *"Prompts can be added in many places"* — the separate "In my words" block became three prompt slots placed between photos and sections (Hinge interleaves prompts through a profile), and the After-you-connect preview now alternates photos, prompts and biodata the same way. (3) *"The color combinations are really bad"* — the neutrals were blue-tinted navy, and every saffron tint on a dark surface rendered brown, producing a navy/orange/brown clash. Neutrals are now hue-free greys (a faint warm lean in light mode only), the saffron background washes are gone, the Next-up card is neutral with the accent only on its dot, and selected chips are solid saffron with near-black text instead of a tinted fill. | Profile photos + prompts + palette v3 — krishna kategaru (autonomous), 2026-09-14. |
| 2026-09-14 | **Profile hub — per-capability pass (manual check → research → improve → retest), as the product owner directed.** Capabilities: hero/completion ring, Next up, photos, prompts, answer sections, Edit/Preview, settings, log out. Findings and changes so far: **Completion ring** — verified live that it links to `/me/completeness`; that screen already matches palette v3. **Photos** — research (Bumble help) showed people reorder by long-press and drag; Mangaly only had "make main". Added migration `013-profile-media-sort-order.sql` (additive `sort_order`, backfilled primary-first), `PATCH /profile/photos/order` (`profile.reorder_photos`, rejects any list that isn't exactly the caller's current photos once each), and one shared rule — position 0 is the main photo — now used by make-main, reorder and delete (deleting the main photo promotes the next in order, not the oldest). Grid: 350 ms long-press starts a drag (a move over 8 px first is treated as a scroll), drop target outlined, optimistic reorder with server rollback on failure; the photo sheet gains Move earlier / Move later as the non-drag alternative (WCAG 2.5.7). Found live: the reorder request was blocked by CORS because the service allows only GET/POST/PATCH/DELETE; switched the endpoint from PUT to PATCH rather than widening the global CORS list. Verified via API: reverse order applied with exactly one primary, original restored, mismatched list returns 409. **Prompts** — Hinge caps text prompts at 150 characters; the answer limit was 200, now 150. **Answer sections** — Hinge treats partner preferences as private filters never shown to others, but the After-you-connect preview displayed them; they are now excluded from the preview and the Partner preference section says "Only you see this". Research items noted but not built (larger scope): Hinge voice/video prompts and prompt polls; dealbreaker toggles on preferences. **Photo reorder, verified by hand in the browser:** long-press drag enters drag mode, highlights the drop target, swaps the grid, and the server order matches with exactly one main photo first. Two real defects were found and fixed during that check: (a) the long-press timer called `setPointerCapture` before setting drag state, so any failed capture (pointer already released or cancelled) aborted the drag entirely; state is now set first and the capture is guarded; (b) a finished drag never fires a click, so the "swallow the click that ends a drag" flag stayed on and the first genuine tap after any reorder was silently ignored; the flag is now cleared at the start of every press, and a drag-then-tap check confirms the photo sheet opens. The sheet's Move earlier / Move later buttons were verified too (the swap is saved on the server and the grid matches), with Move later correctly disabled on the last photo. **Settings and log out:** research (logout placement studies) found a header log-out button invites accidental taps, and the inline settings card pushed page content down. The gear now opens a settings bottom sheet (language shown in each language's own script, appearance, then Log out behind a confirm step), and Log out is gone from the header. | Profile per-capability pass — krishna kategaru (autonomous), 2026-09-14. |
| 2026-09-14 | **Profile hub — per-capability pass, round 2: answer sheets, hiding, languages, dark theme.** Every item was checked live in Chrome DevTools against the seeded account, and each saved value was confirmed in `mangaly_profile.profile_attribute`. The seed data was then restored. **Partner age range:** added a dual range slider above the exact number fields (Baymard: a slider for a fast approximate range, number fields for precision). Verified: 2 sliders + 2 fields, values start at 28/35, dragging one handle past the other clamps both to 33/33, the track fill follows the handles, and Save persisted `{min:26,max:33}` (seed 28–35 restored). **Occupation:** the text field now offers a `<datalist>` of 20 common roles, and free text is still accepted. Verified that 'Data Scientist' persisted, then the seed value 'Product Manager' was restored. **Single-select (Pets):** tapping a chip saves at once and closes the sheet; the tile and the section count (2/4 → 3/4) update in place; `love_pets` persisted. **Prefer not to say:** the field becomes `declined` with a null value, and the tile shows 'Hidden' with its muted style (test row removed afterwards). **Hindi:** no missing hi/te keys in the profile or common namespaces (script diff against en); the only Latin text on `/me` in Hindi is user-entered content. **Dark theme — defect found and fixed:** in the settings sheet, the selected segment (`--surface-raised` #161618) was *darker* than the sheet (#242427), so 'English'/'Dark' looked recessed rather than chosen. Added a `--surface-container-highest` tier (light #fff, dark #36363b) for `.segmented__opt.is-active`; verified that sunken (17) < sheet (36) < selected (54). **Second defect:** the system-preference dark block still used the old navy sunken tone `#101a2f`, so only system-dark users got a blue tint; it now matches the explicit dark theme (#111113). **Seed hygiene:** a canvas-generated test photo left over from upload testing had become the main photo; its row and file were removed and the original seeded photo is main again. **Research candidate, not built yet:** Hinge lets people keep an answer for matching while hiding it from their profile. Here, 'Prefer not to say' discards the value. A per-field 'visible on profile' toggle would keep matching quality without exposure. | Product owner: "do manual check for each capability in the screen for profile and research every time what can be improved" |
| 2026-09-14 | **Profile hub — per-capability pass, round 3: interest badges, one editing surface, pinned Save.** Each item was manually checked in Chrome DevTools and each saved value confirmed in Postgres; seed data was restored afterwards. **Food, travel & hobbies:** the 'Next up' card led to an empty free-text box. Verified research: Bumble's 'My interests' lets members pick up to five of about 200 Interest Badges in categories such as Sports, Food & drink and Traveling ([Bumble](https://bumble.com/en/the-buzz/bumble-badges)). Built a new `tags` field kind: 20 India-relevant interest chips (cooking, trekking, pilgrimages, cricket, classical dance, spirituality…), a live 'Pick up to 5 · N chosen' counter, and an 'Add your own' field (Enter or Add). Stored as a JSON list: known keys are saved as keys, so they translate; custom text is stored verbatim. Older comma-separated answers are still read: seeded Karthik Iyer's `"Trekking, cooking, cricket"` opens as three selected chips, and his tile reads 'Trekking, Cooking, Cricket'. Verified for Ananya: at five picks the other 15 chips and the custom field lock; deselecting frees a slot; Enter added 'Carnatic music'; Save stored `["cooking","trekking","cricket","reading","Carnatic music"]` (jsonb array); the tile updated; Next up moved on to Pets. Chip labels and sheet strings were added to en/hi/te. **One editing surface:** the completeness screen still linked to the old full-page editor (`/me/edit/<category>`), a second, older way to edit. Its links now go to `/me?edit=<category>`, which opens the same bottom sheet and cleans the URL back to `/me`. The old route is now a redirect, so bookmarked links keep working: `/me/edit/pets` landed on `/me` with the Pets sheet open. **Pinned Save:** with 20 chips the sheet scrolls, and Save was below the fold (measured off-screen). `.sheet__actions` is now sticky at the sheet's bottom edge with a hairline divider; Save is fully in view while the chips scroll. **Considered and deferred — per-field 'visible on profile' toggle (Hinge pattern):** attributes are served only to their owner (`/profile/attributes/full`, `/profile/{category}`), and no screen renders another person's biodata answers yet, so the toggle would protect nothing today. It is scheduled with the viewer-facing profile detail. **Seed hygiene:** Karthik's dev password no longer matched (likely changed by earlier password-reset testing; the API returned 401 while Ananya's returned 200). His hash was reset to the shared dev password; login verified 200. | Product owner: "do manual check for each capability in the screen for profile and research every time what can be improved" |
...
| 2026-09-14 | **ForKhatri platform identity integration (parent `docs/ParentApp/07-tech-reqs.md` TR10–TR17, TR23; DEC-V1-015).** Mangaly's interim sign-in is retired: MangalyService now accepts only the ForKhatri `fk_session` cookie, resolved by the Identity Bridge against the platform Identity & Trust Service (:8100), and `mangaly_identity.account` is the member-link table (migration 014, applied). Interim credential routes answer `410 moved_to_forkhatri` behind `INTERIM_IDENTITY_ENABLED=false`; the web client hands sign-in/sign-out to the ForKhatri entrance (:3100) and gains a persistent ForKhatri hub link. pytest 32 → 63 passed (31 new), mypy clean, ruff clean on touched files, `tsc` clean; 11/11 live Playwright checks. See the section of the same name below. | Product owner: one ForKhatri sign-in — krishna kategaru (autonomous), 2026-09-14. |
| 2026-09-14 | **FR006 / FR020 / FR021: viewing a connection's profile, and a photo-privacy hole closed.** Read-map found that every uploaded photo was served from a public static `/media` mount: anyone holding a URL could load any member's photo without signing in or being connected, contrary to FR006 ("media access is denied by default absent explicit authorization"). Backend: `main.py` no longer mounts `/media`; new `api/routes/media.py` serves `GET /media/{profile_id}/{filename}` only when RLS lets the caller read that `profile_media` row (owner, or a `candidate_info` grant from an accepted connection), with a strict filename pattern, a media-root path guard (`storage.local_path`) and `Cache-Control: private, no-store`. New `profile.view_profile_full()` + `GET /profile/view/{account_id}/full` returns the profile, all photos in order and every answered attribute in one response (TR020: no teaser shape, no `locked` field); `partner_preference` is excluded because preferences are private matching filters. `_bind_viewer()` now holds the authorization-context binding shared by `view_profile`, `view_profile_full` and the media check. Web: new `/profile/[accountId]` screen laid out like a Hinge profile (photos interleaved with prompts and biodata sections), a locked state for members who are not connected, and a Message button when opened from a conversation; the conversation header now links to it; answer formatting moved to `lib/profileFormat.ts` so `/me` and the new screen share it; en/hi/te strings added. **Verified live** (API through the ForKhatri session cookie; UI in a dedicated headless Chrome): Ananya (accepted connection) sees Rahul's full view with 1 photo and 7 answered categories, no partner preferences, no `locked` field; Karthik (not connected) gets `null` and the locked screen with no images; Rahul's photo returns 200 `image/jpeg` + `private, no-store` for Ananya, 404 for Karthik, 401 anonymously; conversation header link lands on `/profile/<id>?connection=<id>`. ruff, tsc and eslint clean. Found during the check: the locked title was truncated by the hero-name ellipsis style, switched to a wrapping heading. | FR006, FR020, FR021; product owner: "implement what FRs are left" |
| 2026-09-15 | **FR046 / FR047: selective sharing inside an accepted connection.** Read-map: FR046 lists additional photos, video introduction, additional personal info, phone, email and family contact as independent categories; UX20 specifies one row per category, an explicit confirmation naming what becomes visible and to whom, and a received view; TR046/TR047 require one grant row per category and no automated disclosure. Two schema defects found in the sealed ER model and fixed by **migration 015** (applied): `sharing_grant` had `UNIQUE (connection_id, category)`, so only one side of a connection could ever share a category, and its RLS policy had no `WITH CHECK`, so either participant could write a grant naming the other as the sharer. The key is now `(connection_id, category, granted_by_account_id)`, writes require the caller to be the sharer on an accepted connection, a category check was added, and `shared_value` (plus `family_contact_share.family_member_phone`) stores the value revealed, because Mangaly can read only a member's own platform phone/email, never another member's. Product decision recorded here: accepting a connection unlocks the main photo and the profile; every other photo is "additional photos" and is visible only while shared. `view_profile_full` and the authorized media route both enforce this through `connection.has_active_share()`. Backend: `SharingGrant` model; `connection.sharing_overview()`, `set_share()` (resolves an `AuthzContext`, idempotent, withdrawal clears the stored value) and `has_active_share()`; `GET /connections/{id}/sharing` and `PATCH /connections/{id}/share/{category}` (409 when there is nothing to share). Web: a sharing card on the connection's profile screen (no extra screen): each category is its own row; Share opens an in-place confirmation ("Share your phone number with Rahul Varma? They will see +91…"); shared rows show the date and Stop sharing; rows with nothing to share show only a hint; "What Rahul has shared" lists phone and email as tap-to-call and mail links. Message moved into the header after the check showed the pinned bar sliding under the tab bar, and disabled Share buttons were removed after they rendered brown. en/hi/te strings. **Verified live** (API through ForKhatri sessions; UI in the Mangaly Chrome profile, Rahul in an isolated context): Ananya shares phone → Rahul's overview lists only the phone, no email; repeating the share changes nothing; a stranger gets 404 on the connection; unknown or empty categories get 409; stopping sharing empties Rahul's list. Extra-photo gating with a temporary second photo: Rahul saw 1 photo and got 404 for the extra file before sharing, 2 photos and 200 after, 1 and 404 after withdrawal; a stranger got 404 throughout; the temporary photo and its file were removed. UI: confirmation text named Rahul and the number, the row changed to "Shared Sep 14, 2026" with Stop sharing, Rahul saw a `tel:` link. Outbox shows CategoryShared/CategoryShareWithdrawn for each action. Test shares withdrawn afterwards. ruff, tsc and eslint clean. | FR046, FR047; product owner: "implement what FRs are left" |
| 2026-09-15 | **FR097 (part 1): a real parent can act in the candidate's search; test data brought back to reality.** Product-owner correction that drove this: FR048 had been exercised with a Home Circle membership inserted straight into Postgres that cast Karthik Iyer (another candidate) as Ananya's "parent" after the invite endpoint returned 429. The owner rejected it ("stick to product plan… without parent how could all of above happen? I want you to be in reality"). Corrective actions: the inserted membership was marked removed (never deleted); Ananya's two stale memberships naming her own match Rahul as a "sibling" were marked removed; the invite rate-limit counter left full (20/20) by earlier automated runs was cleared; a real family member was created the sanctioned way (`platform/identity-service/scripts/seed_dev_members.py`: Lakshmi Reddy, +919999900003, Ananya's mother, not a candidate); Ananya invited her from the Circle screen, Lakshmi signed in and accepted, which created the membership through `invitation_id` and the `family_info` grant. The earlier FR048 verification is superseded and will be repeated with Lakshmi. Gap found by being realistic: after accepting, Lakshmi had no way to act for Ananya, and Home and Me sent her (a member with no candidate profile) into the candidate profile wizard. Built per FR097/UX06: **migration 017** `mangaly_home_circle.list_member_contexts()` (SECURITY DEFINER, caller's own active memberships only, candidate name + relationship, because a family grant does not unlock the candidate's profile row); `home_circle.list_member_contexts()` and `GET /home-circle/contexts` (own profile + circles); web `lib/activeContext.ts` (chosen context remembered per device, re-validated against the signed-in member's contexts) and `components/ContextChip.tsx` (always states whose search is active in words; violet, the colour reserved for acting for someone else; switch sheet with a live-region announcement when more than one context exists); Home and Me route a member with no candidate profile but an active circle to `/circle` instead of the wizard; the Circle screen shows the family view (who they are helping, their role, that anything they do shows as coming from family) and hides candidate-only cards. en/hi/te strings. **Verified live** (Mangaly Chrome profile, separate isolated contexts per person): contexts API returns Lakshmi → no own profile, one circle (Ananya Reddy, parent); Ananya and Rahul → own profile, no circles. Lakshmi opening Home and Me lands on `/circle`, header "Home Circle" with "Viewing Ananya's search" (not switchable, one context), role and explanation shown, invite/members/notes cards hidden. Ananya's Circle shows no chip and her normal screen. A first pass repeated the context in the title ("Helping Ananya"), which wrapped beside the chip on a phone; the title is back to "Home Circle". **Not yet verified:** the switch sheet itself, because no realistic member is both a candidate and in someone else's circle yet; it will be tested when such an account exists rather than faked. **Next:** Discover in the family context (FR013) with Suggest (FR014), then family notes (FR016). ruff, tsc and eslint clean. | FR097, FR013 prerequisite; product owner: "start" |
| 2026-09-15 | **FR013 / FR014 in the family context: a parent searches and suggests for the candidate; the candidate could never see suggestions (fixed); a privacy leak caused by an earlier test shortcut (fixed).** Built: `discovery.authorize_family_viewer()` (resolves `Capacity.FAMILY` and requires `family_info` on the candidate); `discovery.search()` and `compatibility.explain()` accept `for_candidate_account_id`, rank against the candidate's own snapshot and exclude both the candidate and the family member; `GET /discovery/search` and `GET /discovery/compatibility/{id}` take `for_candidate` (403 without the grant). Web: Discover reads the active context; in family mode it shows the context chip, "Searching for Ananya. Your suggestions go to Ananya, who decides.", hides incoming requests (the candidate's own decisions), fetches results and reasons for the candidate, and the primary action is a worded "Suggest to Ananya" button (`SwipeDeck` `rightText`) that calls `POST /home-circle/suggest`, never a connection request; the privacy line reads "Identity protected until Ananya connects"; education levels show their labels ("Master's degree") on Discover and on suggestion cards; "Suggested by your parent" uses the lowercase relationship word. **Bug found by testing with a real parent, fixed by migration 018:** Ananya's own `GET /home-circle/suggestions` returned [] although her mother's suggestion existed. `hc_suggestion_family` admitted only `has_scope(candidate,'family_info')`, which a candidate never holds on herself, so the person every suggestion is for could never see one, contrary to `list_suggestions()`'s documented self-read and FR014. Added a SELECT-only policy `hc_suggestion_candidate_read` (`is_self(candidate_profile_id)`); writes stay family-only. **Leak found while verifying, caused by my own earlier shortcut:** Rahul (Ananya's match) could read her circle's suggestions. Two memberships that had been marked `removed` by a direct database update still had active `family_info` grants, because that update bypassed `_end_membership()`, which revokes the grant by `source_reference_id`. Revoked both grants; a sweep now finds no active grant on any ended membership. The product path was correct; the shortcut was not, which is why test data is now only created through the app. Also removed three test suggestions authored by those fake memberships. **Verified live:** Lakshmi (parent) searching for Ananya → 200, 6 results, neither Ananya nor Lakshmi included; reasons for a result match Ananya's own; Karthik (not in the circle) → 403 on search and reasons. Browser: Lakshmi's Discover shows the chip, family line, no incoming requests, "Suggest to Ananya"; tapping it stored a suggestion under her parent membership and showed "Suggested to Ananya. Nothing was sent to that person."; Ananya's Discover is unchanged (no chip, Connect); after migration 018 Ananya's API and Circle screen show "Suggested by your parent … no action has been taken on your behalf"; after revoking the grants Rahul and Karthik read [] and Lakshmi still reads the suggestion. **Open:** one seeded candidate (Anita Rao) stores occupation as `software_engineer`, which Discover shows verbatim; it should be corrected through that member's own profile, not by editing data. A member with no candidate profile can still call `GET /discovery/search` without `for_candidate` (snippets only; the UI never does this); to be closed with FR025. Next: family notes (FR016). ruff, tsc and eslint clean. | FR013, FR014, FR097; product owner: "start" |
| 2026-09-15 | **Home Circle members shown by name.** Ananya's Members list showed only "Parent" and a date. A family member such as Lakshmi has no Mangaly candidate profile; her name is the ForKhatri `display_name`, which Mangaly's Identity Bridge was discarding. **Migration 019**: `mangaly_identity.account.display_name`, and `mangaly_home_circle.list_circle_members()` (SECURITY DEFINER; only the candidate's own active members; name from the link row, else the member's Mangaly profile). The Identity Bridge now keeps `display_name` from the session claims and writes it to the member's own link row inside their own request (allowed by `account_self_only`, only when it changed), so the frozen `ensure_platform_account()` is untouched. `GET /home-circle/members` returns `member_name`; the Circle screen shows the name, an initials avatar, and "Parent · Joined <date>" (en/hi/te). **Verified live:** after Lakshmi's next request synced her name, Ananya's members API returned `member_name: Lakshmi Reddy` and the Circle screen shows "LR · Lakshmi Reddy · Parent · Joined 9/14/2026"; Rahul's own circle stays empty. 32 identity-bridge tests pass; ruff, tsc, eslint clean. | FR007–FR010 display, FR097; product owner: "start" |
| 2026-09-15 | **DEC-V1-016: Discover cards lead with the candidate's main photo and age.** Product owner, comparing the text-only card with a Shaadi.com card: "at least one pic should be shown to user, without that what will they see? It is just time waste." Recorded in `v1-decisions.md`; name, additional photos, contact details and full biodata stay consent-gated. **Migration 020**: `mangaly_discovery.discovery_cards(ids)` (searchable candidates only: main-photo reference and age) and `mangaly_profile.is_discoverable_primary_photo(ref)`, both SECURITY DEFINER and requiring a signed-in caller. Backend: `discovery.cards_for()`; `GET /discovery/search` and `GET /discovery/snippet/{id}` return `photo_url` and `age`; `profile.media_path_for_viewer()` serves a searchable candidate's main photo to any signed-in member, while non-main photos still need ownership or an accepted, shared connection; `profile.resolve_photo_url()` added; the `SearchResult` "never name or photo" docstring updated. Web: photo-first swipe card (photo fills the card; "33 yrs · Bengaluru, Karnataka" headline; education · profession; "Name and contact are shared only after you connect" / "…after Ananya connects"); taller deck; incoming requests and Circle suggestion cards show the photo; an unloadable photo falls back to the placeholder instead of a broken-image icon; en/hi/te. **Verified live:** Lakshmi searching for Ananya → 6 results, 5 with `photo_url` and age; a searcher loads a main photo (200 image/jpeg); anonymous → 401. Browser: Rahul's card ("33 yrs · Bengaluru, Karnataka", "Bachelor's degree · Software Engineer") loads his real photo, but it sits third in the deck; the cards on top are the test accounts described below, whose unrenderable images now fall back to the placeholder. **Found (not changed yet, needs the owner):** (1) the first three cards are the same "Anita Rao" three times with 149-byte placeholder images: they are not seed people but accounts left behind by the automated check `scripts/check_profile_categories.py`, which signs up a new "Anita Rao" on every run and never cleans up; (2) Ananya's search also returns women, because search has no filter for the gender a candidate is looking for. ruff, tsc and eslint clean. | DEC-V1-016, FR021, FR026; product owner: "at least one pic should be shown" |
| 2026-09-15 | **Test data cleanup: removed accounts left behind by automated check scripts.** Product owner: "so your test scripts should be proper." `scripts/check_profile.py` and `scripts/check_profile_categories.py` had each signed up a new candidate on every run ("Priya Sharma" ×6, "Anita Rao" ×3, each with a 149-byte placeholder JPEG) and `check_auth.py`/`check_otp.py` had left 6 unnamed sign-ups; none were ever removed, and the three "Anita Rao" copies sat on top of Discover with unrenderable photos. Removed in one guarded transaction per database (the transaction aborts if any listed account has a Home Circle membership, message or grant): Mangaly — 15 accounts, 9 profiles, 19 attributes, 9 media rows, 9 Discovery index rows, 4 pending connection requests that pointed at them, 1 suggestion that pointed at one of them, 20 sessions, 16 OTP challenges; the 9 placeholder photo folders; ForKhatri identity service — the same 15 members and their 15 password credentials, guarded to phones the scripts generate. Milavn's own development persona ("Priya Sharma", no phone, milavn email) was deliberately kept. **Consequence:** Lakshmi's one suggestion made through the UI pointed at a removed test account, so the suggestion flow will be re-verified against a real candidate. **Scripts:** both profile check scripts still sign up through the retired interim identity path (`INTERIM_IDENTITY_ENABLED=false`), so they cannot run today; they are to be rewritten to sign in through ForKhatri as a dedicated development member and delete everything they create. | Test hygiene; product owner: "your test scripts should be proper" |
| 2026-09-15 | **DEC-V1-017: matches follow each member's own "Looking for" answer, asked while creating the profile.** Product owner: "it comes based on their preference … they give who you're interested in, as per that it should be coming" and "those details needed to be taken while filling profile". The documents were silent on gender (M01-G leaves the hard-filter taxonomy open). Built: profile creation requires `looking_for` (`bride`/`groom`, form field with a strict pattern) and stores it as `partner_preference.looking_for`; the wizard's step 2 asks "Looking for: Bride / Groom" beside gender and Next stays disabled until both are chosen; the Partner preferences sheet lists "Looking for" first and the tile shows e.g. "Groom · Age 28–35". The Discovery snapshot now carries `gender` and `looking_for`; `search()` keeps a profile only when the candidate is who the seeker looks for AND the seeker is who the candidate looks for, before DEC-V1-002 ranking; a seeker without an answer gets 409 `looking_for_missing`, shown as "Tell us who you're looking for" with Choose now (opens the partner sheet), or for a family member "Ananya hasn't said who they're looking for yet". Discovery reindexed. en/hi/te. **Verified live:** before answering, Ananya's own search and Lakshmi's search for her returned 409 and both prompts rendered correctly in the browser; Ananya answered Groom through Choose now → Partner preferences → Save; Rahul and Karthik answered Bride through their own sessions (the same PATCH the sheet sends); then Ananya → Rahul, Karthik; Lakshmi for Ananya → Rahul, Karthik; Rahul → Ananya; Karthik → Ananya (never each other). The spare development member was routed to profile creation and the wizard's step 2 behaved as above; nothing was saved for that member. 2 profile/discovery tests pass; ruff, tsc, eslint clean. **Browser check completed after a server outage:** the ForKhatri identity service (8100) had stopped; after restarts, the Mangaly API, the identity service and Milavn's API were force-stopped within seconds, even when launched detached. The cause was another session's build agent killing every python.exe; that session (forkhatri-f3) changed its agent to stop only its own process, and the restarted services then stayed up. Verified in the Mangaly Chrome profile: Ananya's Discover shows Rahul ("33 yrs · Bengaluru, Karnataka", "Bachelor's degree · Software Engineer") and Karthik ("36 yrs · Chennai, Tamil Nadu", "Master's degree · Chartered Accountant") with their photos; Lakshmi viewing Ananya's search sees the same two with "Suggest to Ananya"; Rahul sees only Ananya ("31 yrs · Hyderabad, Telangana") with her photo. Milavn's API (8001) was left for the Milavn session to restart. | DEC-V1-017, FR026, FR027; product owner: "as per that it should be coming" |
| 2026-09-15 | **Discover: filters instead of a search box, age range in ranking, card sized for a phone.** Product owner: "discover don't have any search, but only have filters, like nearby etc … read the Shaadi.com docs for the filters"; searching for and adding people stays on the Circle page; matches come from our engine per the .md files and .docx. Research: Shaadi.com's own FAQ says matches come from the member's profile and partner preferences, and its refine options include photo availability, marital status, mother tongue, religion, country/state/city, education and profession; our UX16 specifies a filter sheet (Location & relocation, Lifestyle, Partner preferences) and no search box; DEC-V1-002 keeps preferences as ranking signals. Built: `discovery.DiscoverFilters` (same city / same state relative to the seeker's own locality, age from/to, marital status, education, lifestyle/diet, open to relocating) applied after the mutual "looking for" rule and before ranking, never re-ranking; the snapshot now carries `marital_status` and `date_of_birth`; `_score` now counts the preferred age range alongside preferred locality in the 30% partner-preference term (the earlier "age range not used" scope cut is closed); the free-text `locality` query parameter was removed; `GET /discovery/search` takes the new filter parameters (403/409 behaviour unchanged). Web: the "Filter by locality" box is replaced by a Filters button (shows the active count) and a quick "Same city" chip; a filter sheet with Location, Age, Marital status, Education and Lifestyle chips, Reset and Show matches; en/hi/te. The swipe deck is sized so the card text and actions sit above the tab bar on a phone. Photo availability is not offered as a filter because every searchable profile must have a photo (DEC-V1-001). Discovery reindexed. **Verified via the API as Ananya** (female, looking for a groom; Hyderabad): no filters → Rahul, Karthik; same city → none; age 30–34 → Rahul; never married → Rahul; education Master's → Karthik; open to relocating → Rahul; Lakshmi searching for Ananya with age 35–40 → Karthik. ruff, tsc, eslint clean; 2 profile/discovery tests pass. **Verified in the browser as Ananya** (647 px tall viewport): the filter bar shows Filters and Same city with no locality box; the sheet shows Location, Age, Marital status, Education and Lifestyle with translated chips plus Reset and Show matches. The first card height left the buttons behind the tab bar, so the empty error line above the deck is now rendered only when there is an error, the deck is `clamp(260px, 100dvh − 350px, 560px)`, and the photo is anchored at 25% from the top so the face stays in frame; measured buttons bottom 552 px vs tab bar top 587 px, with the face and details visible. | UX16, FR025–FR027, DEC-V1-002; product owner: "only have filters" |
| 2026-09-15 | **Family contact sharing ends when its reason ends; `is_self` is NULL-proof; check scripts rebuilt.** Found live by the platform session: a family contact row still named another candidate (my earlier direct test insert) with a phone after the grant was revoked. The row was deleted. Root causes fixed in migration 021: (1) withdrawing a family contact now also deletes its request rows; (2) when a Home Circle membership ends (removed or left), `end_family_contact_for_member` deletes that member's request and share rows and revokes the candidate's grant carrying their phone (called from `_end_membership`); (3) `mangaly_authz.is_self` returned NULL instead of false for someone else's id, so PL/pgSQL guards written `IF NOT is_self(...)` (migration 016's `decide_family_contact_share`) let a third party through; it now always returns true or false, and RLS policies keep their meaning; (4) a shared parent's name was empty because parents have no Mangaly profile, so it now falls back to the ForKhatri display name. The `Account` model gained `display_name` (migration 019), which fixed the schema-conformance test. Check scripts: `check_profile.py` and `check_profile_categories.py` no longer use the retired `/auth/signup` + OTP-log flow or a 149-byte image. They sign in through ForKhatri as the automated-test member, upload a real portrait, include "looking for" (and check it is required), refuse to run if that member already has a profile, and delete the profile, attributes, photo rows and files, and Discovery row in `finally` (`scripts/_check_support.py`). New `check_family_contact.py` walks the real flow and restores the starting state. Three unreferenced 149 B/1 KB leftover photo files from old script runs were removed. **Verified:** `check_profile` 10/10, `check_profile_categories` 13/13, `check_family_contact` 11/11 (Ananya requests, Lakshmi approves in her own session, Rahul sees name, relationship and phone; nothing ends while Lakshmi is a member; Rahul cannot end it; Lakshmi leaving ends it, run in a rolled-back transaction; withdrawing clears it); afterwards the member has no profile, 0 share rows and 0 active family contact grants remain; pytest 65 passed; ruff, tsc and eslint clean. Older scripts (`check_otp`, `check_password_reset`, `check_discovery_connection`) still use the retired interim sign-in and are not run. | FR048/TR048, FR010, FR001–FR005; platform session finding |
| 2026-09-15 | **FR016 rebuilt: private family notes about a match (DEC-V1-018).** Product owner: "what is this family notes? Improve and innovate here". Read FR016, UX13/UI13, TR016, TS035–TS037, M01-B §12, M01-D and M01-I. The old build had no match on a note, let every family member read every other member's notes (against TS035), had no request step (every note was simply pending for the candidate, who approved blind) and no decline. Built: migration 022 (`subject_account_id`, `forward_requested_at`, `forward_declined_at`, `updated_at`; 1,000-character and forward-order checks; RLS `hc_note_author` and `hc_note_candidate_reads_shared`; SECURITY DEFINER `is_active_note_author`, `list_note_forward_requests`, `decide_note_forward`, `list_shared_notes`; the old `list_own_pending_notes` and `approve_note_forward` dropped; three junk rows from a retired script removed). API: `POST/GET /home-circle/notes`, `PATCH/DELETE /notes/{id}`, `POST /notes/{id}/request`, `GET /notes/requests`, `POST /notes/{id}/read|decline`, `GET /notes/shared`; events `FamilyNoteReadRequested`, `FamilyNoteRead`, `FamilyNoteDeclined` (no content in payloads). Web: `FamilyNotes` panel on a match's profile when acting for a candidate (write, edit, delete, ask with an in-place preview of exactly what the candidate will read, status: only you / waiting / read / said not now); a family member who is not connected now sees the match's pre-connection card (photo, age, place, education, profession) instead of "send a request from Discover"; a "Private note" link on Discover's family card; Circle's "Notes from family" card shows who asks and which match (links to the profile), Read note / Not now, and notes already read. Hindi and Telugu strings for all of it, plus the backend's missing Hindi/Telugu catalogs for Home Circle, connection and message errors (they had fallen back to English). **Verified via the API** (`scripts/check_family_notes.py`, 21/21, real people, cleans up): blank note and a note about the candidate rejected; Ananya and Rahul cannot list Lakshmi's notes; a request shows author, relationship and match with no content; Rahul cannot read it; Ananya reads exactly that note and only that note is shared; a shared note cannot be rewritten; "not now" shows as declined with no reason, cannot be re-asked unchanged, and editing makes it private again; every note deleted afterwards. Hindi and Telugu error text returned for `X-Mangaly-Language`. **Verified in the browser** (Lakshmi and Ananya in their own sessions): Lakshmi opened Karthik from her Discover context, saw his card, saved a note, asked with the preview, saw "Waiting for Ananya"; Ananya's Circle showed Karthik's card and "Lakshmi Reddy (Parent) wants you to read a note about this match" with the words absent from the page; Read note showed the words with "From Lakshmi Reddy (Parent)"; Lakshmi then saw "Ananya read this" with only Delete offered; deleting removed it from both sides (0 notes left). The Discover "Private note" link opens the match's profile without swiping the card. pytest 65 passed; ruff, tsc, eslint clean. `scripts/check_home_circle.py` still uses the retired `/auth/login` and the old notes API and is not run. | FR016/TR016, UX13, TS035–TS037, M01-B §12, M01-I; DEC-V1-018 |
| 2026-09-15 | **TR026 completed: diversity re-rank and a two-way partner-preference term.** Read DEC-V1-002, TR026, SP026 and `reference.md`'s match logic ("2-way Matches: both sides' preferences match each other"). Two gaps: DEC-V1-002's diversity re-ranking pass did not exist, and the 30% partner-preference term only checked what the searcher wants of the candidate, so someone outside what they themselves are looking for ranked as highly as a two-way fit. Built: `_preference_checks()` runs in both directions (the searcher's preferred locality and age range against the candidate, and the candidate's against the searcher; every stated preference counts equally, unstated ones add nothing); `_diversify()` runs after the weighted sort, before paging, so no more than 3 results in a row share one of 10 equal score bands. When a fourth would repeat, the highest-ranked result from another band moves up; nothing is dropped and order inside a band never changes. The constants live in `config/ranking_weights.py` (`SCORE_BRACKETS`, `MAX_SAME_BRACKET_RUN`), version `DEC-V1-002.v2`; the weights are unchanged and popularity stays 0. **Verified:** new `tests/unit/test_discovery_ranking.py` (5 passed): the run rule on a long top band, a single band left unchanged, SP026's contract on a representative 60-result set (rule holds, nothing lost, order within bands kept), unstated preferences, and a one-way fit scoring half a two-way fit. Live: Ananya and Lakshmi-for-Ananya now get identical rankings (Rahul 0.375, Karthik 0.225), and Rahul and Karthik each still see only Ananya. pytest 70 passed; ruff clean. No browser check: nothing on screen changed. Also removed a stale pending connection request an old script left: Rahul, whose membership in Ananya's Home Circle had since been removed, "on behalf of" Ananya's profile, sent to Karthik. Karthik would have seen a request in Ananya's name that she never made (see DEC-V1-019). | TR026, SP026, DEC-V1-002, FR026 |
| 2026-09-15 | **DEC-V1-019: family members suggest; only the candidate sends a connection request (the suspected on-behalf accept bug, confirmed and closed).** Confirmed by reading migration 010: accepting grants profile visibility between the request's sender account and the recipient. For a request sent "on behalf of" a candidate, a parent without a profile left both sides with no visibility; a family member who is also a candidate would have been given the wrong person's visibility; and the candidate became connected with someone they never chose. The development data held a live case: a removed circle member's pending request in Ananya's name to Karthik (removed). The sources decide the rule (M01-D §7: a relative's natural action is to raise a suggestion; M01-I §6–7: suggestion is not consent, initiation authority is not decision authority; M01-B / M01-D §9: connect is the candidate's own action). Built: `connection.send_request()` refuses any `on_behalf_of_profile_id` with `OnBehalfNotAllowed` (422, translated: "Family members suggest profiles. Only the candidate can send a connection request."); the old `unauthorizedOnBehalfOf` string is replaced in en/hi/te. No screen sent on-behalf requests; family Discover already uses Suggest. **Incident, reported plainly:** the first live check ran against a stale API. The dev server's auto-reload had hung after a test file was saved, so the old code kept serving. That check created one real on-behalf request (Lakshmi for Ananya, to Karthik); it was deleted straight away, before anyone could see it. The API on port 8000 (this session's own process) was stopped by exact PID and restarted. **Verified on the restarted API:** Lakshmi (in Ananya's circle) and Rahul (not in it) are both refused in English, Hindi and Telugu, and no row is created (14 requests before and after, none on behalf). Regression sweep on the fresh process: check_profile 10/10, check_profile_categories 13/13, check_family_contact 11/11, check_family_notes 21/21; Discover unchanged (Ananya and Lakshmi-for-Ananya: Rahul 0.375, Karthik 0.225; Rahul and Karthik each see only Ananya); afterwards 0 notes, 0 family contact rows, 0 active family contact grants, no test-member profile, 14 connection requests. `authorization.resolve_effective_account()` now has no caller; it is kept for a future candidate-confirmed on-behalf flow. pytest 70 passed; ruff clean. | FR042/TR042, FR043/FR044, M01-D §7–9, M01-I §6–7; DEC-V1-019 |
| 2026-09-17 | **Discover is a scrolling feed; acceptance opens the conversation; a match's biodata is readable before connecting; a conversation is a session.** The product owner's batch of 2026-09-17. **Discover (items 7, 8):** the one-card swipe deck is replaced by a scrolling feed — each row leads with the candidate's main photo, and the next page loads as the reader nears the end (the deck never asked for page 2, so a session was capped at the first 20 people). `lib/discovery.ts` gained the `page` parameter and reports whether more exist; `app/discover/page.tsx` is a list with an IntersectionObserver sentinel; feed styles in `globals.css`; en/hi/te strings. One always-visible photo was already true (DEC-V1-016) and is unchanged. **Acceptance (item 2):** migration 026 adds `message.kind` (member/system) with a NULL sender for system lines, plus `open_conversation()`; `connection.accept()` now opens the conversation and puts one system line in it, so the candidate learns a request succeeded where they will actually talk. The line stores a key, not prose, so it renders in the reader's own language. **Parent vs candidate view (item 4, DEC-V1-020):** migration 027's `public_biodata()` and `GET /profile/view/{id}/public` expose a searchable candidate's attributes (never name, contact or extra photos); the profile screen renders a Shaadi-style biodata sheet for a family member and the Hinge-style read for a candidate. **Session conversation (item 5, DEC-V1-021):** `POST /messages/{id}/close` deletes everything the two wrote; the thread ends the session on pagehide and visibilitychange and via an explicit End chat and delete button, and states plainly that a browser cannot prevent screenshots. **Seed data:** the four Hyderabad families were completed — 16 accounts imported into the platform identity service, and the missing `seed_dev_family_circles.py` written and run, creating all 12 memberships through the real invite-and-accept flow so every `family_info` grant is issued by product code. That script is HTTP-only on purpose: importing a database driver costs minutes on this machine, which made an earlier version look hung. ruff, tsc and eslint clean. **Pending verification:** the Mangaly API is slow to restart under machine load, so the new endpoints and the role-by-role browser pass are not yet checked. | FR016/FR021/FR042/FR044/FR049/FR050, DEC-V1-016/020/021; product owner 2026-09-17 |
| 2026-09-17 | **Voice introduction and voice input; four seeded families verified role by role; two live defects fixed.** **Voice (item 9):** migration 029 adds `audio` to `profile_media.media_type`; `POST/GET/DELETE /profile/voice-intro` record, read and remove one clip per profile (webm/ogg/m4a/mp3, 5 MiB cap, replaced rather than collected); `media_path_for_viewer` serves a recording only to the owner or an accepted connection — hearing a voice is identity-revealing, so it is deliberately not part of the pre-connection card. Web: a recorder on the profile hub beside the photos (`VoiceIntro`, MediaRecorder, one minute, re-record or delete), a player on a connection's profile (`VoiceIntroPlayer`), and `VoiceInput` — a dictation button using the browser's own speech recognition in the reader's language (hi-IN/te-IN/en-IN), wired into the private-notes field, and absent where the browser has no such API. en/hi/te throughout. **Verified live:** recording uploads (201) and reads back; the owner and the accepted connection can play it (200); a stranger gets 404; a non-audio upload is refused (422); deleting removes it (0 audio rows left). The recorder card renders on the hub. **Seed data verified in the app:** signed in as Rajesh Sharma (parent) — Circle shows "Viewing Meera's search", the three members and the requests sent for her; Discover shows the scrolling feed with photos and per-profile actions; a match's profile shows the Shaadi-style biodata sheet with no name. Signed in as Ishita Malhotra (candidate) — the same match reads Hinge-style: photo, her own prompts, then grouped sections, no name. **Two defects found and fixed:** (1) the seeded written answers were stored under a `self_expression` category no screen renders — 12 rows converted to the product's own `in_my_words.prompt_N = {q, a}` model and `seed_dev_families.py` corrected, so a re-run writes them where they are read; (2) migration 028 — `message`'s DELETE policy was sender-only, so ending a session cleared only your own messages and, worse, DEC-V1-014's 30-day purge had never deleted the other party's messages at all; either participant may now clear their own conversation. Re-ran the end-to-end check: 14/14. **Environment note (not product):** this machine's WMI service stopped answering, and Python's `platform` module queries WMI at import (SQLAlchemy calls it), so the API hung on start and every `wmic`/`taskkill` timed out. A `sitecustomize.py` in the Mangaly virtualenv makes the WMI probe fail fast; startup went from hanging to 1.3 s. It is a local environment repair, not repository code — delete it once `Restart-Service winmgmt` is run. ruff, tsc and eslint clean. | FR004/FR006/FR016/FR049/FR050; product owner 2026-09-17 |
...
| TR006 | Profile & Completeness | **Yes (local-storage equivalent)** — `GET /media/{profile_id}/{file}` authorizes every request through RLS (owner or `candidate_info` grantee), `Cache-Control: private, no-store`; the public static `/media` mount is removed. A real Object Storage backend still owes the short-TTL signed URL. |
| TR020 | Profile & Completeness | **Yes** — `GET /profile/view/{account_id}/full` returns profile, photos and non-private answers in one response; no `locked` field; web screen `/profile/[accountId]` |
| TR046 | Connection & Sharing | **Yes (3 of 6 categories)** — `PATCH /connections/{id}/share/{category}` writes one `sharing_grant` row per sharer per category (migration 015); additional photos, phone and email are live. Video introduction and "additional personal info" have no content to share yet and are not offered; family contact is TR048. |
| TR047 | Connection & Sharing | **Yes** — only the explicit sharer call writes a grant; in-place confirmation names the category, the recipient and the exact value; `CategoryShared` / `CategoryShareWithdrawn` outbox events per action. The recipient notification waits for the notification inbox (FR098). |
...
DEC-V1-015 (2026-09-14): this slice is retired as a sign-in mechanism —
see "ForKhatri platform identity integration" below. Its code is kept and
reachable only with `INTERIM_IDENTITY_ENABLED=true`.
**Review history** — (none yet)
**Approval:** Engineering Manager / Tech Lead — [ ] Approved — name, date

---

## ForKhatri platform identity integration — parent TR10–TR17, TR23
**Traces from:** `docs/ParentApp/07-tech-reqs.md` TR10, TR11, TR12, TR14, TR15, TR16, TR17, TR23, TR24; `docs/ForKhatri-Unified-Umbrella-App-Interpretation.md`; `v1-decisions.md` DEC-V1-015 (supersedes DEC-V1-012 as a Mangaly mechanism; pays down IA092)
**Status:** Complete, live-verified (2026-09-14) | **Confidence:** High

**Why.** ForKhatri is one app with one sign-in; modules are entered after it.
The platform Identity & Trust Service (`platform/identity-service`, :8100,
database `forkhatri_identity`) now owns members, credentials, OTP and
sessions, and TR23 imported every active Mangaly account with its id
unchanged, so `member_id == account_id`.

**What's built.**
- *Database* — `07a-db-implementation/migrations/014-platform-identity-link.sql`,
  applied as `mangaly_owner` and re-run once to confirm idempotency:
  `credential_hash` nullable; `account_has_identifier` allows an
  identifier-less row only when `status='deleted'`; SECURITY DEFINER
  `mangaly_identity.ensure_platform_account(member_id, phone, email)` (pinned
  `search_path`, EXECUTE granted only to `mangaly_app`, advisory lock per
  member) creates the link row just-in-time, syncs phone/email, retires a
  conflicting `pending_verification` account, and raises `MGL09
  platform_identity_conflict` / `MGL03 platform_account_not_active` rather
  than guess. `schema.sql` is not updated: it has not mirrored migrations
  003–013 either, and `init.sh` runs `migrations/`.
- *Identity Bridge* — `components/identity_bridge/platform.py` (internal):
  shared `httpx.AsyncClient` (2 s timeout) → `POST /internal/v1/sessions/resolve`
  with `X-ForKhatri-Service`/`-Key`; per-token cache keyed by SHA-256, 30 s
  positive / 5 s negative; `session_invalid` → invalid; timeout, 5xx, our own
  key rejected, malformed claims → unavailable (never cached). `interface.py`
  adds `resolve_platform_session()` (claims → `ensure_platform_account` →
  `set_account_context`) and the exception types; the client is closed on
  shutdown (`main.py`).
- *Settings* — `platform_identity_url`, `platform_service_name`,
  `platform_service_key`, `platform_session_cookie_name`,
  `platform_identity_timeout_seconds`, `forkhatri_entrance_url`,
  `interim_identity_enabled` (default **False**); placeholders in
  `mangaly-service/.env.example`, dev values in the git-ignored `.env`.
- *API* — `deps.get_authenticated_account` reads `fk_session` first (401 /
  503 `identity_unavailable` / 403 link refused); the `mangaly_session`/bearer
  path only when the flag is on. Sign-up, login, login OTP, OTP verify/resend
  and reset request/confirm carry `dependencies=InterimOnly` → `410
  {"detail": {"code": "moved_to_forkhatri", "message", "entrance_url"}}`.
  `/auth/me` now uses the shared dependency. No route code deleted.
- *Web* — `lib/platform.ts` (entrance URL, `redirectToEntrance`, platform
  sign-out); `lib/api.ts` `apiFetch` (credentials include; 401 → entrance with
  `return_to`) used by every `lib/*.ts` call; `getSession` returns null only
  on 401 and throws otherwise (a 503 must not bounce a signed-in member
  through the entrance in a loop), `requireSession` for signed-in screens;
  home shows an "unavailable, try again" state. `/login`, `/signup`, `/otp`,
  `/reset` render `EntranceRedirect` (return_to = Mangaly home, avoiding a
  loop); the old screens stay in the files as `Legacy*` components. Log out →
  `POST :8100/v1/auth/sign-out` → entrance. `ForKhatriHubLink` in the top bar
  of Home, Discover, Circle, Messages and My profile (Mangaly's `.icon-btn`
  style). en/hi/te strings; `.env.example`/`.env.local`.

**Tests.** `tests/unit/test_platform_identity_client.py` (cache hit, TTL
expiry, negative cache, no raw token in cache, request shape, client
headers, unreachable/5xx/bad-key/malformed → unavailable, 503 over HTTP,
revoked → 401, legacy credential ignored with the flag off, 410 on all seven
credential routes) and `tests/integration/test_platform_identity_link.py`
(real DB as `mangaly_app`: new member → link row + RLS context + own
identifiers; identifier sync; pending holder retired; active holder refused;
identifier-less claims fail closed; `fk_session` end to end; interim bearer
accepted only with the flag on). Full suite **63 passed** (was 32); mypy
clean; ruff clean on every touched file (68 pre-existing findings elsewhere
unchanged). Web: `tsc --noEmit` clean; `npm run lint` shows one
pre-existing error (`app/otp/page.tsx` set-state-in-effect, identical on
HEAD).

**Live verification, not asserted** (Playwright 1.63, 390×844; the entrance
at :3100 was not running, so it was stubbed at the network layer to observe
the exact redirect URL). 11/11: signed-out `/me` →
`:3100/?return_to=http%3A%2F%2Flocalhost%3A3000%2Fme`; `/login` → entrance with
`return_to` = Mangaly home; member `1111…` (Seed Candidate One) sees Mangaly
home; hub link present and navigates to :3100; new member `a0000000-…-0001`
lands in `/profile/create`, completes all four steps and reaches home;
log out → entrance, cookie cleared, the revoked token gets `401` from
MangalyService after 20.4 s (cache bound 30 s); a Mangaly screen after
log out → entrance with `return_to`. `curl` `/auth/login`, `/auth/signup`,
`/auth/otp/verify`, `/auth/reset/request` → 410. Postgres: link row for the new
member with `+919999900001` / `krishna.dev@forkhatri.test`, `active`,
`credential_hash` NULL, and profile "Krishna Kategaru". Screenshots:
`mangaly-web/.logs/platform-identity/01…10-*.png` (+ `results.json`). The
photo in `07-…` shows as broken only because the uploaded test fixture is a
149-byte stub JPEG from earlier scripts, served 200 `image/jpeg`.

**Contract check against the running identity service.** No contradiction
found: resolve returned TR14 `SessionClaims` including `phone_e164`/`email`
for `mangaly`; a bogus token → `401 {"detail":{"code":"session_invalid"}}`; a
wrong key → `401 {"detail":{"code":"service_unauthorized"}}` (same status,
distinguished by `code`, which the client relies on).

**Constraints surfaced / open.**
- `scripts/check_auth.py`, `check_otp.py`, `check_password_reset.py` (manual
  scripts, not pytest) exercise the interim routes and now get 410 unless the
  API runs with `INTERIM_IDENTITY_ENABLED=true`.
- Signed-in screens other than Home stay on their loading state when the
  API/identity service is unreachable (no loop, but no explicit message).
- Dormant interim tables and credential hashes remain until a reviewed
  cleanup migration (DEC-V1-015 trade-off).
- TR22 `basePath`/Multi-Zones remains a scheduled module task.
- Sealed BR/FR/UX/UI text still describes Mangaly-owned login; corrected by
  the separate documentation pass, not here.
