...
| 2026-09-14 | Post-seal correction: ForKhatri platform identity. Added dated notes under the `mangaly_identity` traceability section and Assumptions #1: `mangaly_identity.account` becomes Mangaly's member-link table keyed by the canonical platform `member_id` (existing ids preserved); credentials, OTP challenges, reset tokens and sessions are no longer written for platform members. No DDL is changed by this note. Not re-sealed; awaits the owner's review. | Product-owner instruction, 2026-09-14. See `docs/ParentApp/07-tech-reqs.md` TR10–TR11, TR23 and `docs/ParentApp/07a-er-model.md`. |
...
> **2026-09-14 correction:** `account` becomes Mangaly's member-link table: its `id` is the canonical ForKhatri `member_id` (active accounts imported with ids preserved, TR23), created just-in-time on first entry (TR11). For platform members Mangaly no longer writes credentials, `otp_challenge`, `password_reset_token` or `session` rows; those tables remain for existing data and automated tests of the disabled interim routes. Sign-in data lives in `forkhatri_identity` (docs/ParentApp/07a-er-model.md). Any schema change to reflect this (for example relaxing credential columns) is module implementation work, not made here.
...
1. **`mangaly_identity` schema (interim).** `architecture.md` §2.1 states
   ...
   **Migration plan**, per `v1-decisions.md`: this schema is retired and its
   ...
   > **2026-09-14 correction:** the platform module now exists
   > (`platform/identity-service`). The plan differs from "retire the schema":
   > the `account` table stays as Mangaly's member-link table with preserved
   > ids; active accounts' identifiers and Argon2id hashes are copied to
   > `identity.member`/`identity.password_credential`; pending accounts are not
   > imported (PA-DEC-08, TR23).
