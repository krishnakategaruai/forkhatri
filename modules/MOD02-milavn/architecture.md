...
| 2026-09-14 | Implementation note only (no decision re-opened): the Identity & Trust edge is now wired to the real platform session per `docs/ParentApp/07-tech-reqs.md` TR10–TR17 — see the note under §1's edge table. | Product owner: one ForKhatri sign-in, one member identity. |
...
**Identity & Trust edge as implemented (2026-09-14, `docs/ParentApp/07-tech-reqs.md` TR10–TR17).** The edge is realised as an opaque ForKhatri session, not a JWT: the browser holds one HttpOnly `fk_session` cookie, and Milavn's Identity Bridge (`milavn-service/app/components/identity_bridge/interface.py`) resolves it server-to-server via `POST /internal/v1/sessions/resolve` (service name + key), caching 30 s positive / 5 s negative so a platform sign-out reaches Milavn within 30 s; display names for other members come from `POST /internal/v1/members/lookup` (60 s cache, "Community member" fallback). The platform unreachable on an authenticated request is a 503 (fail closed). Module roles (`milavn.moderate`) stay Milavn data, keyed by `member_id`. `milavn_profile.member_profile` remains the member-link table, created lazily by onboarding. The `X-Milavn-Member-Id` development stand-in is off unless `DEV_IDENTITY_ENABLED=true` (tests only). Record: `AgentOutputs/09-implementation.md` IMP25.
