...
| 2026-09-14 | Post-seal correction: ForKhatri platform identity. Added dated correction notes under TR46 and TR47: Milavn's Identity Bridge resolves the `fk_session` cookie through the platform internal API instead of accepting the development `X-Milavn-Member-Id` header (disabled by default, test-only setting); signed-out members are redirected to the ForKhatri entrance; logout is platform sign-out. Requirement bodies and approvals unchanged. Not re-sealed; awaits the owner's review. | Product-owner instruction, 2026-09-14. See `docs/ParentApp/07-tech-reqs.md` TR11–TR16 and `/MODULE-ARCHITECTURE-STANDARD.md` §5b. |
...
> **2026-09-14 correction:** Milavn does not call sign-up/log-in/reset/OTP endpoints at all; those flows are delivered by the ForKhatri entrance and Identity & Trust Service (docs/ParentApp/07-tech-reqs.md TR12–TR16). Milavn's Identity Bridge reads `fk_session`, resolves it via `POST /internal/v1/sessions/resolve` (cache ≤30 s, fail closed `503`), ensures `milavn_profile.member_profile` exists, and binds its RLS context (TR15, MODULE-ARCHITECTURE-STANDARD §5b). The development `X-Milavn-Member-Id` / `dev_identities.json` path is disabled by default and allowed only by an explicit test setting; development identities were imported with their ids (TR23). Permission priming (FR081/FR082) is unchanged.
...
> **2026-09-14 correction:** logout calls `POST ${IDENTITY_API}/v1/auth/sign-out` then navigates to the entrance; every `401` navigates to `${FORKHATRI_ENTRANCE_URL}/?return_to=…`; the shell shows a persistent way back to the ForKhatri hub (docs/ParentApp/07-tech-reqs.md TR16). The platform-wide deletion orchestration this TR flags is still not specified in the platform contract.
