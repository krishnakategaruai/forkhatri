...
| 2026-09-14 | **Step 9 product pass, migrations 011–012.** 011 replaces `milavn_notification.deliver()` with a 24-hour duplicate guard (same member, class, title, source occurrence) so re-joins and retried jobs do not fill the inbox; the Important-class never-muted rule (FR051) is unchanged. 012 adds `locality_city` / `locality_locality` (both nullable, approximate places per FR038) and an index to `milavn_circle.circle` so Discover circles can list the viewer's locality first; seeds anchor the four development circles. No RLS policy changed. | Product-owner pass — krishna kategaru (autonomous). |
| 2026-09-14 | **Beyond-MVP, migrations 013–014.** 013 adds `milavn_activity.occurrence_message` (thread), `milavn_activity.occurrence_photo` (moments), `milavn_circle.circle_post` (board), all RLS-enabled with policies that delegate to two new definer helpers — `can_view_thread(occ, member)` (organizer, co-organizer, or RSVP'd) and `was_there(occ, member)` (organizer or checked-in/attended) — plus `circle_peer_ids()`, `milavn_profile.public_card()` and `milavn_activity.hosted_public_ids()` for the person page. 014 adds `milavn_profile.public_locality()` because the `member_public_profile` view inherits the base table's self-only RLS; the service clamps the result to the person's own precision (FR041). No existing policy changed; `milavn_app` still owns nothing. | Beyond-MVP pass — krishna kategaru (autonomous). |
| 2026-09-14 | **Messaging, migration 015** — schema `milavn_connect`: `conversation` (direct/group), `conversation_member` (last_read_at, muted), `chat_message` (text/photo/expression), `message_reaction`, `presence` (last_seen_at, expression, at_occurrence_id). Definer helpers: `is_conversation_member`, `can_message` (shared active circle or shared occurrence incl. organizer↔participant, and no block), `direct_conversation_id`, `create_conversation` (the only write path for conversations/members), `touch_conversation`, `heartbeat`, `presence_of`, `unread_count`. RLS on all five tables delegates to the helpers; `milavn_app` has SELECT/INSERT/UPDATE only where a person acts on their own rows. | Owner decision — krishna kategaru (autonomous). |

| 2026-09-13 | **Step 9 fix-forward migrations 002–008** (no table or column changed; `001-initial.sql` untouched). 008: the TR17 co-participation job writes suggestion rows for *other* members against `circle_formation_suggestion_member`'s member-self policy — definer-owned `create_formation_suggestion`/`suggestion_member_ids`/`suggestion_exists_for_pair`. 009: the FR052 reminder job's candidate read (`milavn_notification.reminder_candidates()`), same dispatcher pattern as 004. 010: `GRANT UPDATE ON milavn_trust.feedback` — FR066 lets a member revise their own feedback and the application upserts; the member-self policy still scopes it. Implementation found that several already-cited requirements could not be met from an ordinary request under the Sealed RLS policies, which are themselves correct: FR006/FR008 aggregate counts over *other* members' private participation rows; FR037 qualitative reputation over the internal-only `reputation_signal`; FR018/FR019/TR39 organizer-side and promotion writes against the participant-self `WITH CHECK`; Notification Dispatch reads after commit with no acting member (FR017/FR057/FR054); FR041 another member's precision level. Each is resolved with a `SECURITY DEFINER` helper owned by `milavn_owner` (the same pattern this file already established for `milavn_circle.is_active_member`), returning only counts/labels/ids — never a row another member could not otherwise see. Also: TR39's lock is a transaction-scoped advisory lock (a participant cannot `FOR UPDATE` a row they may not UPDATE), and `reputation_labels()` was corrected to `array_append` (006). Recorded here so the ER model's live implementation and this document do not drift. | Step 9 implementation — krishna kategaru (autonomous), 2026-09-13. |
